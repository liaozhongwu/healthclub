$(function(){
	activity_modify_init();
	$(".activity").submit(activity_update_submit);
});
function activity_modify_init(){
...
function activity_addSession(){
	...
	$.post("model/activity_session.jsp",{"num":num},function(data){
		button.before(data).attr("nextSession",Number(num)+1);
		activity_modify_init();
		stepTwo_dropdown_init();
...
function activity_update_submit(){
